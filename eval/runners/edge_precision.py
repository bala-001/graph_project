...
from eval.datasets import load_dataset, gold_edges, dataset_is_synthetic  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="paiq-eval-") as tmp:
        cfg = Config(provider="mock", d_enabled=True, journal_dir=tmp)
        for record in records:
            doc = extract_document(record["document_id"], record["chunks"], cfg)
            d_by_type = {t: set() for t in EDGE_TYPES}
            gt_by_type = {t: set() for t in EDGE_TYPES}
    ...
    per_type = {
        t: {
            "precision": _ratio(tally[t]["hit"], tally[t]["d"]),
            "recall": _ratio(tally[t]["hit"], tally[t]["gt"]),
            "precision_lower": lower_bound(tally[t]["hit"], tally[t]["d"]),
            "recall_lower": lower_bound(tally[t]["hit"], tally[t]["gt"]),
            "n_d": tally[t]["d"],
            "n_gt": tally[t]["gt"],
        }
        for t in EDGE_TYPES
